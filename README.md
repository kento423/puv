...
本プロジェクトでは、Vercelの無料プラン制約を考慮し、**最小構成（main / develop の2ブランチ運用）**を採用しています。

- **main ブランチ**
  - 本番環境用ブランチ（VercelのProductionにデプロイ）
  - 常に安定状態を保つこととし、直接コミットは禁止です。

- **develop ブランチ**
  - 開発用ブランチ（VercelのPreview環境にデプロイ）
  - featureの統合先として使用します。軽微な不具合修正もここで行います。

- **feature/* ブランチ**
  - 機能開発用ブランチ
  - 必ず `develop` から作成してください。
  - リモートにはpushせず、ローカルブランチのみで運用します。
...
## 📊 ポケモンステータス（Lv1〜15）の同期手順

新ポケモンのステータス取得や、既存データの精度向上のために、Unite-DB の API からステータス情報を同期します。

### 1. ステータスの自動取得

以下のコマンドを実行すると、`unite-db.com` の内部 API から全ポケモンの Lv1〜15 の全ステータスを一括取得し、`prisma/pokemonStatSeedData.json` を更新します。

```bash
# JSONの更新を実行
npx tsx scripts/sync-stats.ts

# 更新せずに差分や取得結果だけ確認したい場合
npx tsx scripts/sync-stats.ts --dry-run
```

### 2. データベースへの反映

取得した JSON データをデータベースに反映します。

```bash
npm run seed
```

※ `seed` コマンドはポケモン本体のデータ（Master Data）とステータスデータの両方を同期します。

⸻

## 🛠 パッチ（環境のアップデート）管理手順

ポケモンユナイトのバトルバランス調整（アプデ）に合わせて、掲示板の投稿を時系列でグルーピングするための「パッチ」を管理します。

### 新しいパッチを追加する

新しいアプデが入った当日、または情報公開時に以下のコマンドを実行します。

```bash
npx tsx scripts/add-patch.ts "2026-04-16 16:00"
```

このコマンドを実行すると：
1.  現在「最新」となっているパッチが「過去パッチ」に変更されます。
2.  入力した日時をベースに新しい「最新パッチ」が作成されます。
3.  以降の掲示板への投稿（環境ポケモン、BAN候補、バグ報告）は、自動的にこの新しいパッチに紐づけられます。

※引数の日時は `YYYY-MM-DD HH:MM` 形式で入力してください。
